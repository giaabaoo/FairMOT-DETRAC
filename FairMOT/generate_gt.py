...
def gen_labels_detrac(data_root, ann_root, label_root, chosen_videos, chosen_annotations):
    ...
    for i, xml_file_path in enumerate(chosen_annotations):
        ...
        Path(os.path.join(label_root, video_name,'gt')).mkdir(parents=True, exist_ok=True)
        label_fpath = os.path.join(label_root, video_name,'gt','gt.txt')
        my_dict = {}
            ...
            for attr in target_list:
                if type(attr) is list:
                    for obj in attr:
                        ...
                        # pdb.set_trace()
                        label_str = '{:d},{:d},{:.6f},{:.6f},{:.6f},{:.6f},1,-1,-1,-1\n'.format(
                            int(chosen_frame),tid_curr, left , top , width, height)
...
if __name__ == '__main__':
    data_root = './DETRAC_dataset/fairmot_detrac_dataset/images/train'
    ann_root = './DETRAC_dataset/DETRAC-Train-Annotations-XML-v3'
    label_root = './DETRAC_dataset/fairmot_detrac_dataset/images/train'

    folders = np.array(os.listdir(data_root))
    
    chosen_videos = folders
    ...
    data_root = './DETRAC_dataset/fairmot_detrac_dataset/images/val'
    ann_root = './DETRAC_dataset/DETRAC-Train-Annotations-XML-v3'
    label_root = './DETRAC_dataset/fairmot_detrac_dataset/images/val'
